...
export function webhooksRouter(): Router {
  ...
  router.post("/api/webhooks/payment/:brandId", async (req, res) => {
    ...
    if (status === "captured") {
      const paid = updateOrder(order.id, { status: "paid", paymentAdapterRef: ref ?? order.paymentAdapterRef }, "Payment confirmed by connected processor");
      if (paid) await bookDeliveryIfConnected(paid, brand);
    } else if (status === "refunded") {
      updateOrder(order.id, { status: "refunded" }, "Refunded by connected processor");
    } else if (status === "failed") {
      updateOrder(order.id, {}, "Payment failed at connected processor");
    }
    res.json({ ok: true });
  });

  router.post("/api/webhooks/delivery/:brandId", (req, res) => {
    const brand = getBrandById(req.params.brandId);
